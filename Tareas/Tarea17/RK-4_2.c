...
int main() {
    ...
    while (ti < t_max && ri > 6.371e6) { // detener si toca la Tierra (ri = 6371 km)
        double k1_r = f_r(ti, ri, vi);
        double k1_v = f_v(ti, ri, vi);

        double k2_r = f_r(ti + 0.5*h, ri + 0.5*h*k1_r, vi + 0.5*h*k1_v);
        double k2_v = f_v(ti + 0.5*h, ri + 0.5*h*k1_r, vi + 0.5*h*k1_v);

        double k3_r = f_r(ti + 0.5*h, ri + 0.5*h*k2_r, vi + 0.5*h*k2_v);
        double k3_v = f_v(ti + 0.5*h, ri + 0.5*h*k2_r, vi + 0.5*h*k2_v);

        double k4_r = f_r(ti + h, ri + h*k3_r, vi + h*k3_v);
        double k4_v = f_v(ti + h, ri + h*k3_r, vi + h*k3_v);

        double rip1 = ri + (h/6.0)*(k1_r + 2*k2_r + 2*k3_r + k4_r);
        double vip1 = vi + (h/6.0)*(k1_v + 2*k2_v + 2*k3_v + k4_v);
        double tip1 = ti + h;
