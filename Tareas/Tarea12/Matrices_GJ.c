...
int main(int argc, char *argv[]) {
    ...
    for (int k = 0; k < filas && k < cols - 1; k++) {
        if (fabs(A[k][k]) < 1e-12) {
            printf("Advertencia: pivot cercano a cero en fila %d. Saltando columna %d.\n", k, k + 1);
            continue;
        }
    ...
    for (int k = filas - 1; k >= 0 && k < cols - 1; k--) {
        if (fabs(A[k][k]) < 1e-12) continue; 

        printf("\nEliminando hacia arriba en columna %d (pivote en fila %d)\n", k + 1, k + 1);
        for (int i = k - 1; i >= 0; i--) {
            double factor = A[i][k]; 
            for (int j = k; j < cols; j++) {
                A[i][j] -= factor * A[k][j];
        ...
        // Mostrar matriz después de eliminar hacia arriba
        printf("Matriz despues de eliminar hacia arriba en columna %d:\n", k + 1);
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < cols; j++) {
                if (fabs(A[i][j]) < 1e-10) A[i][j] = 0.0;
                printf("%10.4f ", A[i][j]);
